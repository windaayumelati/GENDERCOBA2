...
def plot_kneighbors_scatter(model, x, features, y_test):
    ...
    if len(features) == 1:
        plt.scatter(x[y_test == 1][features[0]], x[y_test == 1][features[0]], marker='o', s=50, c='blue', label='Male')
        plt.scatter(x[y_test == 0][features[0]], x[y_test == 0][features[0]], marker='o', s=50, c='pink', label='Female')
        plt.xlabel(f'{features[0]}')
        plt.ylabel(f'{features[0]}')
        plt.title("Visualisasi K-Nearest Neighbors - Perbandingan Gender")
        plt.legend()
        st.pyplot()
        
    elif len(features) == 2:
        plt.scatter(x[y_test == 1][features[0]], x[y_test == 1][features[1]], marker='o', s=50, c='blue', label='Male')
        plt.scatter(x[y_test == 0][features[0]], x[y_test == 0][features[1]], marker='o', s=50, c='pink', label='Female')
        plt.xlabel(f'{features[0]}')
        ...
        ax.scatter(x[y_test == 1][features[0]], x[y_test == 1][features[1]], x[y_test == 1][features[2]], c='blue', marker='o', s=50, label='Male')
        ax.scatter(x[y_test == 0][features[0]], x[y_test == 0][features[1]], x[y_test == 0][features[2]], c='pink', marker='o', s=50, label='Female')
        ...
        st.warning("Pilih satu, dua, atau tiga fitur untuk visualisasi.")
...
def app(df, x, y):
    ...
    if st.checkbox("Plot K-Nearest Neighbors PCA"):
        model, score = train_model(x_train, y_train)
        st.write("Menggunakan data yang dihasilkan dari model")
